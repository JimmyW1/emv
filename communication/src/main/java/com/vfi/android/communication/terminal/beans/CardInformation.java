...
import com.vfi.android.libtools.utils.DateTimeUtil;
...
public class CardInformation {
    ...
    public boolean isCardExpired() {
        if (expiredDate != null && expiredDate.length() == 4) {
            ...
            LogUtil.d(TAG, "isCardExpired, cur=" + currentYearMonth + " card=" + yearMonth);
    ...
    public boolean isValid() {
        ...
        if (isTrack2Valid() && isTrack3Valid() && isServiceCodeValid() && isPanValid()) {
            return true;
        } else {
            return false;
        }
